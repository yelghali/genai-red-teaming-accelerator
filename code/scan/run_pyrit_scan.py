...
async def run_selected_orchestrator(orchestrator_name: str, target_type: str, page: Page = None):
    ...
    simple_objective = config.SIMPLE_OBJECTIVE
    complex_objective = config.COMPLEX_OBJECTIVE
    
    print(f"\n=== Running {orchestrator_name.replace('_', ' ').title()} on {target_type.upper()} target ===\n")
    
    if orchestrator_name == "prompt_sending":
        await run_prompt_sending_orchestrator(simple_objective, target_type, page)
        ...
        await run_pair_orchestrator(simple_objective, target_type, page)
    elif orchestrator_name == "crescendo":
        await run_crescendo_orchestrator(simple_objective, target_type, page)
    else:
